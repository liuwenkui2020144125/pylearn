...
app = tkinter.Tk()
app. overrideredirect (True)    #不显示透明标题栏
app. attributes('-alpha', 0.7)  #半透明
app. attributes('-topmost', 1)  #总是在顶端
app. geometry('130x25+700+400')  #初始大小与位置
...
still = tkinter . IntVar(value=1)      #是否在运行

def onLeftButtonDown(event):
    app. attributes('-alpha', 0.9)   #开始拖动时增加透明度
...
def onLeftButtonUp( event):
    app. attributes('-alpha', 0.7)     #停止拖动时恢复透明度
    canMove. set(0)
